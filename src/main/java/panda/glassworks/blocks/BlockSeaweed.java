...
public class BlockSeaweed extends Block {
		...
		GameRegistry.register(new ItemBlock(this) {
			...
			public EnumActionResult onItemUse(ItemStack stack, EntityPlayer player, World world, BlockPos pos,
				...
				if (world.getBlockState(pos).getBlock() == this.block)
					return super.onItemUse(stack, player, world, pos.up(), hand, facing, hitX, hitY, hitZ);
	...
	public void breakBlock(World world, BlockPos pos, IBlockState state) {
		...
		if (world.getBlockState(pos.down()).getBlock() == this)
			world.setBlockState(pos.down(), this.getStateForPlacement(world, pos.down(), null, 0, 0, 0, 0, null, null),
					3);
	}
	...
	public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer,
			ItemStack stack) {
		if (world.getBlockState(pos.down()).getBlock() == this && world.getBlockState(pos.down(2)).getBlock() == this)
			world.setBlockState(pos.down(), this.getDefaultState().withProperty(STAGE, 3));
		else if (world.getBlockState(pos.down()).getBlock() == this)
			world.setBlockState(pos.down(), this.getDefaultState().withProperty(STAGE, 1));
	}

	protected final void checkAndDropBlock(World world, BlockPos pos, IBlockState state) {
		boolean flag = true;
		if (this.hasOkAboveSpecial(world, pos) && this.hasOkBelow(world, pos))
			flag = false;
	...
	public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
		...
		if (k == 0 || k == 2) {
			if (rand.nextInt(6) == 0 && world.getBlockState(pos.up(2)).getBlock() == Blocks.WATER
					&& world.getBlockState(pos.down(3)).getBlock() != this) {
	...
	public IBlockState getStateForPlacement(World world, BlockPos pos, EnumFacing facing, float hitX, float hitY,
		...
		if (up.getBlock() != this) {
			if (down.getBlock() == this)
				return getDefaultState().withProperty(STAGE, 2);
			else
				return getDefaultState();
		} else {
			if (down.getBlock() == this)
				return getDefaultState().withProperty(STAGE, 3);
			else
				return getDefaultState().withProperty(STAGE, 1);
		}
	}
