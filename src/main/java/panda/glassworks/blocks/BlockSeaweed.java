...
public class BlockSeaweed extends Block{
	...
	boolean isSurroundedByWaterOrSeaweed(World world, BlockPos pos){
		...
		for(MutableBlockPos blockpos : list){
			...
			if(world.getBlockState(blockpos).getMaterial() != Material.WATER && block != Blocks.PRISMARINE && block != Blocks.SEA_LANTERN){
				return false;
			}
		}
		return true;
	...
	/*return (world.getBlockState(pos.north()).getBlock() == Blocks.WATER || world.getBlockState(pos.north()).getBlock()
			== this) && (world.getBlockState(pos.west()).getBlock() == Blocks.WATER || 
			world.getBlockState(pos.west()).getBlock() == this) && (world.getBlockState(pos.south()).getBlock() == Blocks.WATER 
			|| world.getBlockState(pos.south()).getBlock() == this) && (world.getBlockState(pos.east()).getBlock() == Blocks.WATER 
			|| world.getBlockState(pos.east()).getBlock() == this) && (world.getBlockState(pos.down()).getBlock() == Blocks.SAND ||
			world.getBlockState(pos.down()).getBlock() == this) && (world.getBlockState(pos.up()).getBlock() == Blocks.WATER ||
	...
	public void randomTick(World worldIn, BlockPos pos, IBlockState state,Random random) {
		...
		//if(random.nextInt(2)==0){
			if(isTopBlock(worldIn,pos) && canGrowHigher(worldIn,pos) && worldIn.getBlockState(pos.up(2))==Blocks.WATER){
	...
	public boolean canCreatureSpawn(IBlockState state, IBlockAccess world,
			BlockPos pos, SpawnPlacementType type) {
		return true;
	}
	...
	public void updateTick(World world, BlockPos pos, IBlockState state, Random rand){
		if(this.canBlockStay(world, pos) && this.canBlockStay(world, pos.up()) && world.getBlockState(pos.down(10)) != this) world.setBlockState(pos.up(), this.getDefaultState());
