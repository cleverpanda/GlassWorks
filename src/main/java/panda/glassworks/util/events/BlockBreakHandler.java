	...
	public void onBlockDrops(HarvestDropsEvent event) {
		if(event.getState().getMaterial() == Material.GLASS){
			...
			{	if((event.getState().getBlock() == BlockList.DOUBLE_GLASS_SLAB )){
					...
					if((event.getState().getBlock() == BlockList.DOUBLE_STAINED_GLASS_SLAB )){
						event.getDrops().add(new ItemStack(BlockList.STAINED_GLASS_SLAB,2));
					}else
				event.getDrops().add(new ItemStack(event.getState().getBlock(), 1, event.getState().getBlock().getMetaFromState(event.getState())));
			}
			else*/
				if(!(event.getState().getBlock() == BlockList.GLASS_SLAB) && !(event.getState().getBlock() instanceof BlockGlassSlabBase)){
